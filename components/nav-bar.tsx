...
const NavBar: NextPage<NavBarProps> = ({ onRulesClick }) => {
  ...
  return (
    <div className="rounded-xl bg-gray-200 [backdrop-filter:blur(10px)] box-border max-w-[1720px] w-full flex flex-row items-center justify-between py-2.5 px-[24px] text-left text-5xl text-aliceblue font-aclonica">
      ...
      <div className="flex flex-row items-center justify-center gap-[30px] text-center text-base text-white">
        <Link href="/game-creating-session" className="relative leading-[30.97px] no-underline text-white">Home</Link>
        <div className="relative leading-[30.97px] cursor-pointer" onClick={onRulesClick}>Rules</div>
        <Link href="https://shubhamparkhi.vercel.app" target="_blank" className="relative leading-[30.97px] no-underline text-white">Developer</Link>
        {!currentUser ? (
        <Button
          text="Connect Wallet"
